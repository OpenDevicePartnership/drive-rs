...
- Pick your API surface: blocking (`embedded-hal`), async
  (`embedded-hal-async`), or both. Resolved at generation time — no feature-flag
  soup.
...
- **`src/interface.rs`** — `I2cInterface` / `SpiInterface` implementing
...
- **`src/driver.rs`** — the high-level driver type with `new_i2c` / `new_spi`
  constructors and example methods (`device_id`, `set_enable`, plus `_async`
