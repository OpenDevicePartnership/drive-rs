...
|---|----------|------|-------------|------|
| 1 | 🔴 High | `src/example.ext:42` | Description of the issue | `snippet` |
| 2 | 🟡 Medium | `src/other.ext:10` | Description of the issue | `snippet` |
| 3 | 🟢 Low | `src/lib.ext:5` | Description of the issue | `snippet` |
...
- Do NOT flag formatting, style, or compilation errors — formatters, linters, and CI handle those.
- DO focus on **design**: Is the abstraction correct? Does the change fit the existing architecture? Are there better patterns?
- DO evaluate **concurrency correctness** — especially around async patterns, shared state, and synchronization primitives.
- DO check that conditional/feature-gated code is correct when code depends on optional dependencies.
- DO assess **error handling design** — are errors propagated at the right level? Is the error type appropriate?
- DO consider **resource constraints** — appropriate memory usage and avoiding unnecessary allocations.
